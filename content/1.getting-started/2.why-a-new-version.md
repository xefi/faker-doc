...
Since then, the community has carried the library on as `fakerphp/faker`. It is still the most installed PHP faker by far, but its latest release, v1.24.1, dates back to November 2024, and activity since has been mostly dependency updates. It also inherited the architecture of the 2011 version, and with it the limits we kept running into: a 3.3 MB monolith, extensions that are hard to distribute as standalone packages, and generators that cannot be combined.
