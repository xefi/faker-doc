...
Discovery is not limited to installed packages: Faker also reads the `extra.faker` section of your own project's `composer.json`.
This is handy when your extensions live in the application itself and are not meant to be published as a package:
...
Providers declared this way are registered exactly like the ones coming from installed packages, so everything described below applies to them unchanged.

Faker caches the discovered providers, and rebuilds that cache whenever `vendor/composer/installed.json` or your project's `composer.json` changes. Declaring a provider in your `composer.json` is therefore picked up on the next run, with nothing else to do.
